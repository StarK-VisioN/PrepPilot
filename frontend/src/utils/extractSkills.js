...
/**
 * Extract skill tags from description and/or topicsToFocus.
 * @param {string} input - Primary text (description or topics)
 * @param {{ topicsToFocus?: string, description?: string }} [options]
 * @returns {string[]}
 */
export function extractSkills(input = '', options = {}) {
  const parts = [];
...
/** Merge extracted skills with user-added custom skills (case-insensitive dedupe). */
export function mergeSessionSkills(extractedSkills = [], customSkills = []) {
  const seen = new Set();
  const merged = [];
...
export function isCustomSkill(skill, customSkills = []) {
  if (!skill) return false;
  const key = skill.toLowerCase();
  return customSkills.some((c) => c.toLowerCase() === key);
}

export { SKILL_DICTIONARIES };
